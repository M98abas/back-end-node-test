...
export default class CategoryController {
   ...
   * @returns
   */
  static async getOneWithBrands(req: Request, res: Response): Promise<object> {
    const id = parseInt(req.params.id);
    const data: any = await prisma.category.findMany({
      where: {
        id,
  ...
  static async add(req: Request, res: Response): Promise<object> {
    try {
      ...
      // add item in DB
      const items: any = await prisma.category.create({
        data: {
          title: body.title,
  ...
  static async update(req: Request, res: Response): Promise<object> {
    try {
      ...
      // update data
      const data: any = await prisma.category.update({
